...
form.addEventListener('submit',(e)=>{
    ...
    axios.post('http://localhost:3000/expense/addExpenses',obj,{headers:{'Authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(res=>{
        console.log("res:", res)
...
function displayExpenses(limit,page=1,rows=localStorage.getItem('rows')){
    ...
    axios.get(`http://localhost:3000/expense/getExpense?limit=${limit}&page=${page}&rows=${rows}`,{headers:{'Authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(response=>{
...
displayContainer.addEventListener('click',(e)=>{
    if(e.target.id=='dltbtn'){
        ...
        axios.delete(`http://localhost:3000/expense/deleteExpenses/${trId}`,{},{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
        .then((res)=>{
            tr.remove()
            notifyUser(res.data.message)
        })
...
downloadBtn.addEventListener('click',()=>{
    axios.get('http://localhost:3000/expense/downloadExpenses',{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(response=>{
